...
if [ "$1" = "kill" ]; then
	...
	if [ "${RESULT:-null}" = null ]; then
		echo "Launch"
		HOST=$1 /home/wzx/gpu-moniter/scripts/gpu-run.sh 1 &
	else
		echo "Running"
	fi
	...
	if [ "${RESULT:-null}" = null ]; then
		echo "Launch"
		HOST=$1 /home/wzx/gpu-moniter/scripts/gpu-run.sh 2 &
	else
		echo "Running"
	fi
	...
	if [ "${RESULT:-null}" = null ]; then
		echo "Launch"
		HOST=$1 /home/wzx/gpu-moniter/scripts/gpu-run.sh 3 $2 &
	else
		echo "Running"
	fi
	...
	if [ "${RESULT:-null}" = null ]; then
		echo "Launch"
		HOST=$1 /home/wzx/gpu-moniter/scripts/gpu-run.sh 4 &
	else
		echo "Running"
	fi
fi
